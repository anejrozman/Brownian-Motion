...
v nekem trenutku slu"cajno ustavimo proces in ga gledamo od te točke dalje, bo neodvisen od
dotedanjega gibanja ter bo ponovno Brownovo gibanje. Da to poka"zemo potrebujemo nekaj definicij in pomo"znih rezultatov, ki jih ne bomo dokazovali (dokaz v \cite{3}).
...
\begin{definicija}
    ...
    $$
        \mathcal{F}_T = \{A \in \mathcal{F} \mid A \cap \{T \leq t\} \in \mathcal{F}_t \ \text{za} \ \forall t\}.
    $$
...
\begin{dokaz}
    ...
    \end{align*} 
    Pokazali smo, da velja $B^*_s = B_{T+s} - B_T$. $(B^*_s)_{s\geq0}$ naselednje lastnosti:
    \begin{enumerate}
    ...
    \noindent
    Ko gre $r \rightarrow \infty$ . Po zveznosti realizacij sledi $(B_{T_r + s_k} - B_{T_r + s_{k-1}}) \rightarrow (B_{T + s_k} - B_{T + s_{k-1}})$ $\mathbb{P}$-skoraj gotovo. Po Lebesgueovem izrelu o dominirani konvergenci lahko nesemo limito pod $\mathbb{E}$ in dobimo
    $$
        \mathbb{E}\left[\prod_{k=1}^nf_k\left(B^*_{s_k} - B^*_{s_{k-1}}\right)\mathbbm{1}_G\right] = \mathbb{E}\left[\prod_{k=1}^nf_k\left(B_{s_k} - B_{s_{k-1}}\right)\right]\mathbb{P}(G)
    $$
     $\hfill \square$
...
Tokrat definiramo "cas ustavljanja $T_a = \inf\{t \geq 0 \mid B_t = a\}$ (glej sliko \ref{fig:slika3}, lahko bi izbrali poljuben "cas ustaljanja za katerega velja $\mathbb{P}(T < \infty) = 1$) in v tej vrednosti glede na premico $y = a$ zrcalimo realizacijo Brownovega gibanja (glej sliko \ref{fig:slika6}). 
...
Za zaklju"cek poka"zimo "se idejo dokaza, da je trajektorija Brownovega gibanja nedovedjiva povsod. V celotnem dokazu (glej \cite{1}) uporabimo naslednji rezultat.
...
\begin{izrek}
    trajektorija Brownovega gibanja je neodvedljiva funkcija $\mathbb{P}$-skoraj gotovo, "se ve"c, za vsak $t > 0$ velja ali 
    $$
...
\bibitem{1} R. Durrett, \emph{Probability: Theory and examples} (2019), 353--379. 
\bibitem{2} Tukej bo vir do dokaza za dokaz pomoznih trditev za krepko lastnost markova \emph{Bose-Einstein Condensation in a Gas of Sodium Atoms}, Physics Review Letters \textbf{75} (1995), 3969--3973. 
\bibitem{3} M. Perman \emph{Zapiski predavanj finan"ne matematike 2} (2023)
