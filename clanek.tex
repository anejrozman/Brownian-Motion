...
Vprašanje, ki se porodi je, ali sploh obstaja verjetnostni prostor 
...
\begin{definicija}
    ...
    označimo $\sigma$-algebro generirano s 
...
ali se je $T$ zgodil ali ne (kak"sno vrednost je zavzel $T$). V primeru \ref{ex:NiCasUst} to ne velja, saj moramo vedeti vse, kar se je zgodilo do "casa $b$, da lahko povemo, kje je maksimum. 
...
$$
"Ce velja $B_t \geq 2a - x$, velja tudi $\max_\{B_s\}\geq a$ za $0\leq s\leq t$. Torej je 
$$
    \mathbb{P}\left(\max_{0\leq s \leq t}\{B_s\} \geq a, \ B_t \leq x\right) 
$$
$$
